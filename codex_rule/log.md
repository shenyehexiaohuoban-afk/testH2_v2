...
- Recovered the path generator as ordinary conditional Monte Carlo. The 35 initial states are `a0=2:6`, `loc0=1:7`, `lfw0=0`; each has exactly 15000 records. Intensity, location, and lfw are sampled as separate discrete Markov chains for W1-W3. `path_probability` is the exact product of component transition probabilities and was reconstructed with maximum error below `5e-18`; formal SAA/WDRO nominal records use equal empirical mass `1/15000`, with no importance-sampling correction.
...
- Deterministic tests passed: node tests `7/7`, tree-recursion tests `6/6`, globally enumerated binary microtrees `2/2`, and flat probability-layer size tests `4/4`; random calls were zero. The R=15000 flat layer solved all five eta values in `0.2147415 s` without an R-by-R matrix.
