...
$launch = Join-Path (Split-Path -Parent $runDir) "$RunId-launch-record.txt"
$pidValue = $null
if (Test-Path -LiteralPath $launch) {
    $line = Get-Content -LiteralPath $launch | Where-Object { $_ -like 'MATLAB_PID=*' } | Select-Object -First 1
