function model = build_hourly_lindistflow_h2(data, tau, pElKw, enforceVoltage)
%BUILD_HOURLY_LINDISTFLOW_H2 Build one lossless hourly radial LP.

if nargin < 3 || isempty(pElKw)
    pElKw = zeros(4, 1);
end
if nargin < 4
    enforceVoltage = true;
...
pElKw = pElKw(:);
if numel(pElKw) ~= 4
    error('build_hourly_lindistflow_h2:BadElectrolyzerSize', ...
        'pElKw must contain four site values.');
end
if tau < 1 || tau > 48 || tau ~= floor(tau)
    error('build_hourly_lindistflow_h2:BadHour', 'tau must be 1..48.');
end
...
pvAvailable = data.pv_cap_kw .* data.phi48(tau);
lb(idx.p_pv) = 0;
ub(idx.p_pv) = pvAvailable;

pLoad = data.p_load_base_kw * data.lambda48(tau);
qLoad = data.q_load_base_kvar * data.lambda48(tau);
for i = 1:4
    pLoad(data.site_elec_bus(i)) = pLoad(data.site_elec_bus(i)) + pElKw(i);
end
...
model.Aeq = Aeq;
model.beq = beq;
model.p_load_kw = pLoad;
model.q_load_kvar = qLoad;
model.p_el_kw = pElKw;
model.pv_available_kw = pvAvailable;
model.enforce_voltage = enforceVoltage;
