function run_candidate_formal_h2()
%RUN_CANDIDATE_FORMAL_H2 Formal Stage89Q candidate training entry; OOS is not invoked by the package launcher.
...
checkpoint_metadata=struct('stage','89Q-Pmax-LONG','arm',char(arm),'arm_label',char(label), ...
    ...
    'beta_multiplier',2,'fleet_cap_kg_per_h',160,'eta',0.0, ...
...
policy=struct('representation','modelLib_with_embedded_cuts','state_order','Site1,Site2,Site3,Site4', ...
    'classification','STAGE89Q_PMAX_FIXED_5H_POLICY','candidate_id',char(label),'candidate_pmax_kw',pmax(:));rng_state=rng; %#ok<NASGU>
save_checkpoint_safe(checkpoint,params,modelLib,state,policy,opts,rng_state,checkpoint_metadata);
info=dir(checkpoint);summary=table(label,1000,shortage,p.htt_base_service_cost_yuan_per_kg,2,0.0,opts.seed,budget,trainingWall,iter, ...
    LB(end),finalCuts,totalForward,totalBackward,warningCount,0,initialCuts,string(checkpoint), ...
    info.bytes,fullAudit.pass,"FIXED_5H_BUDGET_TRAINING","NOT_CONVERGENCE_CERTIFICATE",stage1Prod,string(mat2str(pmax.')), ...
...
if string(m.stage)~="89Q-Pmax-LONG"||string(m.arm_label)~=label||~isequal(m.candidate_pmax_kw(:),pmax)|| ...
        ...
        m.training_seed~=20260513||m.training_budget_s~=10800||m.training_wall_time_s<10800|| ...
...
if ~isempty(statusRoot)
    ...
    currentLb=strtrim(getenv('CANDIDATE_CURRENT_LB')); if isempty(currentLb),currentLb='PENDING';end
     statusText=sprintf('CANDIDATE = %s\nPMAX_KW = [%s]\nRUN_ID = %s\nSTATUS = %s\nPHASE = %s\nMATLAB_PID = %d\nCURRENT_ITERATION = %d\nTRAINING_ELAPSED_HOURS = %.9g\nTARGET_TRAINING_HOURS = 5.0\nCURRENT_LB = %s\nLB_CHANGE = PENDING\nCUT_COUNT = %s\nMODELLIB_ESTIMATED_GB = PENDING\nLATEST_VALID_CHECKPOINT = %s\nLAST_SUCCESSFUL_STEP = %s\nLAST_UPDATE = %s\n',candidate,pmax,run_id_from_dir(outDir),status,phase,pid,count,elapsed/3600,currentLb,cutCount,fullfile(outDir,'training','checkpoint','checkpoint_final.mat'),phase,string(datetime('now','Format','yyyy-MM-dd HH:mm:ss')));
