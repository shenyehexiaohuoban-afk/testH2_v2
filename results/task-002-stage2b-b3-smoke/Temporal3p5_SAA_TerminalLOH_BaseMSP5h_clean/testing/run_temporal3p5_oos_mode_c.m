function run_temporal3p5_oos_mode_c()
%RUN_TEMPORAL3P5_OOS_MODE_C Read-only Mode C OOS for the frozen Temporal3p5 policy.

rootDir=strtrim(getenv('CANDIDATE_PROGRAM_ROOT'));
if isempty(rootDir)||~isfolder(rootDir),error('CandidateFormal:Environment','CANDIDATE_PROGRAM_ROOT must point to an existing program root.');end
...
if commit=="",commit="PACKAGE_SOURCE_COMMIT_UNSPECIFIED";end
...
if phase=="BANK",build_oos_bank(rootDir,outDir,commit,pid,false);return;end
...
rows={
    ...
    'terminal_loh_sha256',string(p.terminal_loh_lookup_audit.source_sha256),'b74433...c5fcdc',string(p.terminal_loh_lookup_audit.source_sha256)=="b74433fe8a19caedb75ac3071225480e279c136f3d1285ba503a184f12c5fcdc";
...
T=table(candidate,string(mat2str(pmax.')),1000,opts.seed,18000,string(commit),string(p.terminal_loh_lookup_audit.source_sha256), ...
...
expectedBudget=training_budget_from_env(18000);
allowResume=strcmpi(strtrim(getenv('STAGE89Q_PMAX_LONG_ALLOW_RESUME')),'1');
...
checks=[string(m.stage)=="89Q-Pmax-LONG";string(m.arm_label)==candidate;isequal(m.candidate_pmax_kw(:),pmax); ...
    ...
    m.training_seed==20260513;m.training_budget_s==expectedBudget;m.training_wall_time_s>=expectedBudget; ...
    (allowResume || m.fresh_initial_cut_count==0); ...
    (allowResume && strlength(string(m.warm_start_checkpoint))>0 || ~allowResume && string(m.warm_start_checkpoint)=="NONE"); ...
    string(m.frozen_commit)==string(commit);cut_count(lib,p)==m.cumulative_cuts];
...
resumeCheckpoint=strtrim(string(getenv('STAGE89Q_PMAX_LONG_RESUME_CHECKPOINT')));
if strlength(resumeCheckpoint)>0
    train_arm_resume(rootDir,outDir,arm,pmaxText,commit,pid,char(resumeCheckpoint));
    return;
end
...
budget=training_budget_from_env(18000);started=tic;iter=0;totalForward=0;totalBackward=0;warningCount=0;
...
while iter==0||toc(started)<budget
    ...
    rows(end+1,:)={label,1000,iter,budget,toc(started),lb,delta,sum(fwd.stageCost),before,after,after-before, ...
        ...
        string(warningId),string(warningMessage),"FIXED_3H_BUDGET_TRAINING","NOT_CONVERGENCE_CERTIFICATE", ...
    ...
    setenv('CANDIDATE_CURRENT_CUT_COUNT',num2str(after)); setenv('CANDIDATE_CURRENT_LB',num2str(lb,17));
...
checkpoint_metadata=struct('stage','89Q-Pmax-LONG','arm',char(arm),'arm_label',char(label), ...
    ...
    'htt_c0_yuan_per_kg',htt_base_cost(p),'htt_cost_base_matrix',p.cost_transport_base, ...
    ...
    'training_identity','FIXED_3H_BUDGET_TRAINING','convergence_scope','NOT_CONVERGENCE_CERTIFICATE', ...
...
policy=struct('representation','modelLib_with_embedded_cuts','state_order','Site1,Site2,Site3,Site4', ...
    'classification','STAGE89Q_PMAX_FIXED_3H_POLICY','candidate_id',char(label),'candidate_pmax_kw',pmax(:));rng_state=rng; %#ok<NASGU>
save_checkpoint_safe(checkpoint,params,modelLib,state,policy,opts,rng_state,checkpoint_metadata);
info=dir(checkpoint);summary=table(label,1000,shortage,htt_base_cost(p),2,0.03,opts.seed,budget,trainingWall,iter, ...
    LB(end),finalCuts,totalForward,totalBackward,warningCount,0,initialCuts,string(checkpoint), ...
    info.bytes,fullAudit.pass,"FIXED_3H_BUDGET_TRAINING","NOT_CONVERGENCE_CERTIFICATE",stage1Prod,string(mat2str(pmax.')), ...
...
function train_arm_resume(rootDir,outDir,arm,pmaxText,commit,pid,sourceCheckpoint)
% Continue a completed formal B0001 run in a new result directory.
if arm~="B0001",error('Stage89Q:WarmStart','Only B0001 continuation is permitted.');end
[label,pmax,caseDir,~]=arm_spec(outDir,arm,pmaxText);shortage=200;
if ~isfile(sourceCheckpoint),error('Stage89Q:WarmStart','Resume checkpoint is missing: %s',sourceCheckpoint);end
expectedSha=lower(strtrim(string(getenv('STAGE89Q_PMAX_LONG_RESUME_CHECKPOINT_SHA256'))));
if strlength(expectedSha)~=64||sha256_file(sourceCheckpoint)~=expectedSha,error('Stage89Q:WarmStart','Resume checkpoint SHA mismatch.');end
z=load(sourceCheckpoint);p=z.params;opts=z.opts;lib=z.modelLib;oldState=z.state;oldMeta=z.checkpoint_metadata;oldRng=z.rng_state;
if string(oldMeta.stage)~="89Q-Pmax-LONG"||string(oldMeta.arm_label)~=label||~isequal(oldMeta.candidate_pmax_kw(:),pmax)|| ...
        oldMeta.terminal_gap_penalty_yuan_per_kg~=1000||oldMeta.training_seed~=20260513||oldMeta.fresh_initial_cut_count~=0|| ...
        oldMeta.cumulative_cuts~=cut_count(lib,p)||~isequal(p.el_cap_kw(:),pmax)||~isequal(p.hourly_grid.pmax_kw(:),pmax)
    error('Stage89Q:WarmStart','Resume checkpoint identity failed for %s.',label);
end
assert_model_pmax(lib,pmax,p,'resume checkpoint');
if ~all(isfield(oldState,{'x','theta','lb','LB'})),error('Stage89Q:WarmStart','Resume checkpoint lacks complete training state.');end
sourceTrace=strtrim(string(getenv('STAGE89Q_PMAX_LONG_RESUME_TRACE')));
if strlength(sourceTrace)==0,sourceTrace=fullfile(fileparts(fileparts(sourceCheckpoint)),'iteration_trace','training_progress.csv');end
if ~isfile(sourceTrace),error('Stage89Q:WarmStart','Resume iteration trace is missing: %s',sourceTrace);end
priorTrace=readtable(sourceTrace,'TextType','string');
if height(priorTrace)~=oldMeta.completed_iterations,error('Stage89Q:WarmStart','Resume trace row count mismatch.');end
checkpoint=fullfile(caseDir,'checkpoint','checkpoint_final.mat');tracePath=fullfile(caseDir,'iteration_trace','training_progress.csv');
safe_writetable(priorTrace,tracePath);rng(oldRng);x=oldState.x;theta=oldState.theta;lb=oldState.lb;LB=oldState.LB;rows=table2cell(priorTrace);
priorIter=oldMeta.completed_iterations;priorWall=oldMeta.training_wall_time_s;priorBudget=oldMeta.training_budget_s;initialCuts=oldMeta.fresh_initial_cut_count;
iter=priorIter;totalForward=0;totalBackward=0;warningCount=0;budget=training_budget_from_env(18000);started=tic;
write_status(outDir,'RUNNING','TRAIN_CONTINUE',arm,iter,priorWall,pid,commit);
while toc(started)<budget
    iter=iter+1;iterStart=tic;beforeRows=model_row_counts(lib,p);before=sum(max(0,beforeRows-base_rows(p)),'all');lastwarn('');f0=tic;
    [lib,x,theta,lb,path,fwd]=forward_pass_h2(lib,p,lb,x,theta);forwardTime=toc(f0);assert_model_pmax(lib,pmax,p,sprintf('forward continuation iteration %d',iter));
    valid=all(ismember(fwd.status,["normal","loh_demand_stage","absorbing_lfNc","dissipated_absorb","post_absorb"]));
    if ~valid||~isfinite(lb)||any(~isfinite(x),'all'),error('Stage89Q:Forward','%s continuation iteration %d failed.',label,iter);end
    b0=tic;[lib,cutFlag]=backward_pass_h2(lib,p,x,theta,path);backwardTime=toc(b0);assert_model_pmax(lib,pmax,p,sprintf('backward continuation iteration %d',iter));
    afterRows=model_row_counts(lib,p);after=sum(max(0,afterRows-base_rows(p)),'all');newAudit=audit_new_cuts(lib,p,beforeRows,afterRows);
    if ~newAudit.pass||after<=before,error('Stage89Q:Cuts','%s continuation iteration %d cut audit failed.',label,iter);end
    LB(iter,1)=lb;if iter==1,delta=NaN;else,delta=LB(iter)-LB(iter-1);end;[warningMessage,warningId]=lastwarn;
    if strlength(string(warningId))>0,warningCount=warningCount+1;end;totalForward=totalForward+nnz(fwd.status=="normal");totalBackward=totalBackward+(after-before);
    policyModel=update_rhs_h2(modelLib_at(lib,1,p.k_init),p,p.k_init,1,p.x_0);policySol=solve_stage_model_h2(policyModel);stage1SiteProd=sum(policySol.h2_production_hourly_kg,2);stage1Prod=sum(stage1SiteProd);
    rows(end+1,:)={label,1000,iter,budget,priorWall+toc(started),lb,delta,sum(fwd.stageCost),before,after,after-before,nnz(fwd.status=="normal"),after-before,forwardTime,backwardTime,toc(iterStart),cutFlag,string(warningId),string(warningMessage),"FIXED_CUMULATIVE_BUDGET_TRAINING","NOT_CONVERGENCE_CERTIFICATE",stage1Prod,stage1SiteProd(1),stage1SiteProd(2),stage1SiteProd(3),stage1SiteProd(4),sum(x(:,1)),sum(x(:,2)),sum(x(:,3))}; %#ok<AGROW>
    safe_writetable(cell2table(rows,'VariableNames',training_names()),tracePath);write_status(outDir,'RUNNING','TRAIN_CONTINUE',arm,iter,priorWall+toc(started),pid,commit);
    fprintf('Stage89Q %s continuation_iter=%d additional_elapsed=%.3f cumulative_elapsed=%.3f LB=%.12g cuts=%d\n',label,iter,toc(started),priorWall+toc(started),lb,after);
end
trainingWall=toc(started);fullAudit=audit_all_cuts(lib,p);if ~fullAudit.pass,error('Stage89Q:FinalCutAudit','%s continuation full cut audit failed.',label);end
[stageT,siteT,hourT,gridT,diagSummary]=diagnose_reference_path(lib,p,label);safe_writetable(stageT,fullfile(caseDir,'stage_site','stage_diagnostics.csv'));safe_writetable(siteT,fullfile(caseDir,'stage_site','stage_site_diagnostics.csv'));safe_writetable(hourT,fullfile(caseDir,'stage_site','hour_site_diagnostics.csv'));safe_writetable(gridT,fullfile(caseDir,'grid','hour_grid_diagnostics.csv'));safe_writetable(diagSummary,fullfile(caseDir,'stage_site','reference_path_summary.csv'));
model=first_operating_model(lib,p);finalCuts=cut_count(lib,p);checkpoint_metadata=oldMeta;checkpoint_metadata.training_budget_s=priorBudget+budget;checkpoint_metadata.additional_training_budget_s=budget;checkpoint_metadata.training_wall_time_s=priorWall+trainingWall;checkpoint_metadata.additional_training_wall_time_s=trainingWall;checkpoint_metadata.completed_iterations=iter;checkpoint_metadata.final_LB=LB(end);checkpoint_metadata.cumulative_cuts=finalCuts;checkpoint_metadata.warm_start_checkpoint=sourceCheckpoint;checkpoint_metadata.resumed_from_iterations=priorIter;checkpoint_metadata.training_identity='FIXED_CUMULATIVE_BUDGET_TRAINING';checkpoint_metadata.convergence_scope='NOT_CONVERGENCE_CERTIFICATE';checkpoint_metadata.frozen_commit=char(commit);checkpoint_metadata.variable_count=model.nvars;
params=p;modelLib=lib;state=struct('x',x,'theta',theta,'lb',lb,'LB',LB);policy=struct('representation','modelLib_with_embedded_cuts','state_order','Site1,Site2,Site3,Site4','classification','STAGE89Q_PMAX_FIXED_CUMULATIVE_POLICY','candidate_id',char(label),'candidate_pmax_kw',pmax(:));rng_state=rng;
save_checkpoint_safe(checkpoint,params,modelLib,state,policy,opts,rng_state,checkpoint_metadata);info=dir(checkpoint);
summary=table(label,1000,shortage,htt_base_cost(p),2,0.03,opts.seed,budget,trainingWall,iter,LB(end),finalCuts,totalForward,totalBackward,warningCount,0,initialCuts,string(checkpoint),info.bytes,fullAudit.pass,"FIXED_CUMULATIVE_BUDGET_TRAINING","NOT_CONVERGENCE_CERTIFICATE",stage1Prod,string(mat2str(pmax.')),priorWall,priorBudget,trainingWall,checkpoint_metadata.training_wall_time_s,string(sourceCheckpoint),expectedSha,'VariableNames',{'arm','terminal_gap_penalty_yuan_per_kg','shortage_penalty_yuan_per_kg','c0_yuan_per_kg','beta_multiplier','eta','training_seed','budget_s','actual_training_wall_time_s','completed_iterations','final_LB','cumulative_cuts','forward_operating_solves','backward_cut_solves','warning_count','error_count','fresh_initial_cuts','checkpoint_path','checkpoint_bytes','full_cut_audit_pass','training_identity','convergence_scope','final_stage1_production_kg','Pmax_kw','previous_training_wall_time_s','previous_training_budget_s','additional_training_wall_time_s','cumulative_training_wall_time_s','warm_start_checkpoint','warm_start_checkpoint_sha256'});
safe_writetable(summary,fullfile(caseDir,'acceptance','training_summary.csv'));write_propagation(caseDir,label,pmax,p,lib,checkpoint_metadata,"TRAIN_CONTINUE_PASS");write_text(fullfile(caseDir,'acceptance','TRAINING_FINISHED.txt'),sprintf('%s_TRAINING_FINISHED=true\n',label));write_status(outDir,'RUNNING','TRAIN_CONTINUE_FINISHED',arm,iter,checkpoint_metadata.training_wall_time_s,pid,commit);clear modelLib lib params p z
end
...
requested=str2double(strtrim(getenv('STAGE89Q_OOS_PATH_COUNT')));
if ~isfinite(requested)||requested<1||requested>10000||requested~=floor(requested),error('Stage89Q:OOSCount','STAGE89Q_OOS_PATH_COUNT must be an integer in 1..10000.');end
pathBank=pathBank(1:requested,:);
checkpoint=strtrim(getenv('STAGE89Q_EXTERNAL_CHECKPOINT'));
if isempty(checkpoint),checkpoint=fullfile(trainDir,'checkpoint','checkpoint_final.mat');end
...
if string(m.stage)~="89Q-Pmax-LONG"||string(m.arm_label)~=label||~isequal(m.candidate_pmax_kw(:),pmax)|| ...
        ...
        m.training_seed~=20260513||m.training_budget_s~=18000||m.training_wall_time_s<18000|| ...
        m.fresh_initial_cut_count~=0||string(m.warm_start_checkpoint)~="NONE"|| ...
        string(m.terminal_lookup_sha256)~="b74433fe8a19caedb75ac3071225480e279c136f3d1285ba503a184f12c5fcdc"|| ...
...
chunkDir=fullfile(oosDir,'raw_chunks');if isfolder(chunkDir),error('Stage89Q:OOSOutput','Chunk output exists.');end;mkdir(chunkDir);
...
for q=1:requested
    ...
    if mod(q,100)==0||q==requested
        ...
        chunkPath=fullfile(chunkDir,sprintf('chunk_%05d_%05d',q-nPath+1,q));mkdir(chunkPath);
        write_chunk_tables(chunkPath,pathBuffer,stageBuffer,siteBuffer,hourBuffer,systemBuffer,flowBuffer);
        ...
        batch=table(label,penalty,q-nPath+1,q,nPath,nStage,nSite,nHour,nSystem,nFlow,toc(started), ...
...
pass=height(paths)==requested&&isequal(double(paths.path_id),(1:requested).')&&cutsBefore==cutsAfter&& ...
    checkpointHashBefore==checkpointHashAfter&&maxEq<=1e-6&&maxIneq<=1e-6;
meta=table(label,requested,height(paths),totalSolves,toc(started),cutsBefore,cutsAfter,cutsBefore==cutsAfter, ...
...
write_text(fullfile(oosDir,'OOS_RAW_COMPLETED.marker'),sprintf('%s_%d_OOS_RAW_COMPLETE=true\n',label,requested));
write_status(outDir,'RUNNING','OOS_COMPLETE',arm,requested,toc(started),pid,commit);
end

function write_chunk_tables(chunkPath,pathBuffer,stageBuffer,siteBuffer,hourBuffer,systemBuffer,flowBuffer)
append_table(fullfile(chunkPath,'oos_path_summary.csv'),cell2table(pathBuffer,'VariableNames',path_names()),true);
append_table(fullfile(chunkPath,'oos_stage_summary.csv'),cell2table(stageBuffer,'VariableNames',stage_names()),true);
append_table(fullfile(chunkPath,'oos_stage_site_summary.csv'),cell2table(siteBuffer,'VariableNames',stage_site_names()),true);
append_table(fullfile(chunkPath,'oos_hour_site.csv'),cell2table(hourBuffer,'VariableNames',hour_site_names()),true);
append_table(fullfile(chunkPath,'oos_hour_system.csv'),cell2table(systemBuffer,'VariableNames',hour_system_names()),true);
if ~isempty(flowBuffer),append_table(fullfile(chunkPath,'oos_positive_htt_flows.csv'),cell2table(flowBuffer,'VariableNames',flow_names()),true);end
end
...
for t=1:p.T
    ...
    unit=htt_base_cost(p)+distanceCost;flow=s.f_hourly_kg;costCube=flow.*repmat(unit,1,1,8);
...
[p,opts,candidateAudit]=load_current_stage89_hourly_h2(rootDir,"saa");opts.seed=20260513;
...
if ~candidateAudit.pass||~p.enable_hourly_grid||p.dt_h~=8||p.T~=8||p.Ni~=4|| ...
    ...
    string(p.terminal_loh_mode)~="temporal3p5_saa"||string(p.terminal_loh_lookup_audit.table_version)~="TEMPORAL3P5_SAA_BASEMSP5H_CANDIDATE"|| ...
    string(p.terminal_loh_lookup_audit.source_sha256)~="b74433fe8a19caedb75ac3071225480e279c136f3d1285ba503a184f12c5fcdc"|| ...
    abs(p.terminal_loh_lookup_audit.eta-0.0)>1e-12||string(p.demand_schema)~="original-hourly-24h-repeat-v1"|| ...
...
pass=p.dt_h==8&&p.T==8&&p.Ni==4&&p.hourly_grid.n_operating_stages==6&&p.hourly_grid.hours_per_stage==8&& ...
    ...
    string(p.terminal_loh_mode)=="temporal3p5_saa"&&string(p.terminal_loh_lookup_audit.source_sha256)== ...
    "b74433fe8a19caedb75ac3071225480e279c136f3d1285ba503a184f12c5fcdc";
...
rows(end+1,:)={'training_wall_clock_budget','ALLOWED',true,'18000','18000','PASS'};
...
function budget=training_budget_from_env(defaultBudget)
budget=str2double(strtrim(getenv('STAGE89Q_PMAX_LONG_BUDGET_S')));
if ~isfinite(budget)||budget<=0,budget=defaultBudget;end
end
...
if isfile(path),copyfile(path,[path '.previous.mat'],'f');end
movefile(tmp,path,'f');
copyfile(path,[path '.latest.mat'],'f');
[d,n,e]=fileparts(path);stamp=datestr(now,'yyyymmdd_HHMMSSFFF');copyfile(path,fullfile(d,[n '_' stamp e]));
end
...
assert_arm(arm);
label="TEMPORAL3P5_SAA_BASEMSP5H_CANDIDATE";expected=[300;200;120;150];trainFolder='training';oosFolder='oos_modeC';
...
function p=accepted_bank_path(rootDir)
projectRoot=strtrim(getenv('STAGE89Q_PROJECT_ROOT'));
if isempty(projectRoot),projectRoot=fileparts(fileparts(fileparts(fileparts(rootDir))));end
p=fullfile(projectRoot,'results','task-002-stage2b-b3-smoke','89H-stage85r-single-loc4-stage88-dro-gap1000-10iter-oos10000','run-003','oos','loc4','oos_path_bank.mat');
end
...
statusRoot=strtrim(getenv('CANDIDATE_STATUS_ROOT'));
if ~isempty(statusRoot)
    pmax=strtrim(getenv('STAGE89Q_PMAX_LONG_VECTOR')); if isempty(pmax),pmax='UNKNOWN';end
    candidate=arm; cutCount=strtrim(getenv('CANDIDATE_CURRENT_CUT_COUNT')); if isempty(cutCount),cutCount='0';end
    currentLb=strtrim(getenv('CANDIDATE_CURRENT_LB')); if isempty(currentLb),currentLb='PENDING';end
    statusText=sprintf('CANDIDATE = %s\nPMAX_KW = [%s]\nRUN_ID = %s\nSTATUS = %s\nPHASE = %s\nMATLAB_PID = %d\nCOMPLETED_PATHS = %d\nOOS_ELAPSED_HOURS = %.9g\nLATEST_VALID_CHECKPOINT = EXTERNAL_FROZEN_CHECKPOINT\nLAST_SUCCESSFUL_STEP = %s\nLAST_UPDATE = %s\n',candidate,pmax,run_id_from_dir(outDir),status,phase,pid,count,elapsed/3600,phase,string(datetime('now','Format','yyyy-MM-dd HH:mm:ss')));
    write_text(fullfile(statusRoot,'CURRENT_STATUS.txt'),statusText);
    if phase=="TRAIN_FINISHED",write_text(fullfile(statusRoot,'TRAINING_COMPLETED.txt'),sprintf('TRAINING_STATUS = COMPLETED\nCANDIDATE = %s\nRUN_ID = %s\n',candidate,run_id_from_dir(outDir)));end
end
end
function id=run_id_from_dir(p),[~,id]=fileparts(p);end
...
function value=htt_base_cost(p)
% The accepted hourly builder treats a missing base HTT service field as zero.
if isfield(p,'htt_base_service_cost_yuan_per_kg')
    value=p.htt_base_service_cost_yuan_per_kg;
else
    value=0;
end
end
