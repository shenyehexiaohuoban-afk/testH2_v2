...
httBaseCost = getOpt(opts, 'htt_base_service_cost_yuan_per_kg', NaN);
if ~isscalar(httBaseCost) || ~isfinite(httBaseCost) || httBaseCost < 0
    error('load_data_h2_near:MissingHTTBaseServiceCost', ...
        'opts.htt_base_service_cost_yuan_per_kg must be a finite nonnegative scalar.');
end
params.htt_base_service_cost_yuan_per_kg = httBaseCost;
