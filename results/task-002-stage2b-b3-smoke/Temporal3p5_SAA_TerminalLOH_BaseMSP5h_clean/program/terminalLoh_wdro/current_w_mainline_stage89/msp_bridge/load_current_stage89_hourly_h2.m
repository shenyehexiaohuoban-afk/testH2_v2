...
opts = base_reference_options_h2(rootDir);
...
opts.stage89_bundle_id = 'temporal3p5_saa_base2_v1';

params = load_data_h2_near(opts.dataDir, opts.nearInputFile, opts);
% The canonical Base economic parameter is supplied by this reference
% caller; experiment-specific callers may override it explicitly.
params.cost_reserve_shortage = 1000;
params.NearStageInput.Cost.reserve_shortage_penalty_yuan_per_kg = 1000;
...
params.terminal_loh_mode = "temporal3p5_" + mode;
...
pass = params.dt_h==8 && params.enable_hourly_grid && params.T==8 && ...
    ...
    tableAudit.pass && tableAudit.table_version=="TEMPORAL3P5_SAA_BASEMSP5H_CANDIDATE";
if ~pass
    error('Stage89M:BridgeIdentity', 'Stage89 adopted 8h bridge identity gate failed.');
end
audit = struct('pass',pass,'table_audit',tableAudit, ...
