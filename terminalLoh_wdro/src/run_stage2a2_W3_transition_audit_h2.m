...
outputDir = fullfile(moduleDir, 'output', ...
    'stage2a2_W3_transition_audit', 'run-002');
...
for a = 2:5
    ...
    pStayNew = pStayLegacy + (1/3) * removed - pDownTwo - pUpTwo;
    if pDownTwo > 0
        intFrom(end+1,1) = a; %#ok<AGROW>
        intTo(end+1,1) = a-2; %#ok<AGROW>
        intProb(end+1,1) = pDownTwo; %#ok<AGROW>
    end
    ...
    if pUpTwo > 0
        intFrom(end+1,1) = a; %#ok<AGROW>
        intTo(end+1,1) = a+2; %#ok<AGROW>
        intProb(end+1,1) = pUpTwo; %#ok<AGROW>
    end
end
intFrom = [intFrom; 6; 6; 6];
intTo = [intTo; 4; 5; 6];
intProb = [intProb; 0.02; 0.60; 0.38];
...
%% Generate lfw candidate on states 0:3 with invalid mass added to stay.
lfwDelta = [-1; 0; 1; 2];
lfwKernelProb = [0.05; 0.18; 0.75; 0.02];
lfwRows = {};
lfwCheckRows = {};
for fromLfw = 0:3
    toLfw = fromLfw + lfwDelta;
    valid = toLfw >= 0 & toLfw <= 3;
    invalidMass = sum(lfwKernelProb(~valid));
    normalizedProb = lfwKernelProb(valid);
    validTo = toLfw(valid);
    normalizedProb(validTo == fromLfw) = ...
        normalizedProb(validTo == fromLfw) + invalidMass;
    for jj = 1:numel(validTo)
        lfwRows(end+1,:) = {fromLfw, validTo(jj), normalizedProb(jj)}; %#ok<AGROW>
    end
    lfwCheckRows(end+1,:) = {fromLfw, sum(normalizedProb), invalidMass, ...
...
lfwTransitionChecks = cell2table(lfwCheckRows, 'VariableNames', ...
    {'from_lfw','row_sum','invalid_mass_added_to_stay','min_delta', ...
...
intensityComparisonRows = cell(6, 12);
...
for a = 1:6
    ...
    newDownTwo = optional_prob(intensityRead, 'from_a', a, 'to_a', a-2);
    newUpTwo = optional_prob(intensityRead, 'from_a', a, 'to_a', a+2);
    removed = legacyUp - newUp;
    intensityComparisonRows(a,:) = {a, legacyDown, legacyStay, legacyUp, ...
        newDownTwo, newDown, newStay, newUp, newUpTwo, removed, ...
        newDown-legacyDown, newStay-legacyStay};
    if a >= 2 && a <= 5
        expectedDownTwo = 0.02 * double(a >= 3);
        expectedUpTwo = 0.005 * double(a <= 4);
        intensityMassPass = intensityMassPass && ...
            abs(newUp - 0.2*legacyUp) <= rowTolerance && ...
            abs((newDown-legacyDown) - (2/3)*removed) <= rowTolerance && ...
            abs(newStay - (legacyStay + (1/3)*removed - ...
            expectedDownTwo - expectedUpTwo)) <= rowTolerance;
    ...
    if a >= 3
        downTwoPass = downTwoPass && abs(newDownTwo-0.02)<=rowTolerance;
    else
        downTwoPass = downTwoPass && abs(newDownTwo)<=rowTolerance;
    end
    if a >= 2 && a <= 4
        upTwoPass = upTwoPass && abs(newUpTwo-0.005)<=rowTolerance;
    else
        upTwoPass = upTwoPass && abs(newUpTwo)<=rowTolerance;
    end
end
intensityComparison = cell2table(intensityComparisonRows, 'VariableNames', ...
    {'a','legacy_down','legacy_stay','legacy_up','candidate_down_two', ...
    'candidate_down','candidate_stay','candidate_up','candidate_up_two', ...
    'removed_up_mass', ...
...
checkRows = add_check(checkRows, "INT-02", "intensity", ...
    "intensity changes by at most two levels", ...
    all(abs(intensityRead.to_a-intensityRead.from_a) <= 2), ...
    max(abs(intensityRead.to_a-intensityRead.from_a)), "<=2");
...
checkRows = add_check(checkRows, "INT-06", "intensity", ...
    "run-001 adjacent probabilities retained and long jumps deducted from stay", ...
    intensityMassPass, "formula match", "exact within tolerance");
a6 = intensityRead(intensityRead.from_a==6,:);
checkRows = add_check(checkRows, "INT-07", "intensity", ...
    "a=6 has only transitions to 4, 5, and 6", ...
    height(a6)==3 && isequal(sort(a6.to_a),[4;5;6]), ...
    strjoin(string(sort(a6.to_a)).',','), "4,5,6");
checkRows = add_check(checkRows, "INT-08", "intensity", ...
    "a=6 probabilities are 0.02, 0.60, and 0.38", ...
    abs(find_prob(intensityRead,'from_a',6,'to_a',4)-0.02)<=rowTolerance && ...
    abs(find_prob(intensityRead,'from_a',6,'to_a',5)-0.60)<=rowTolerance && ...
    abs(find_prob(intensityRead,'from_a',6,'to_a',6)-0.38)<=rowTolerance, ...
    "0.02,0.60,0.38", "0.02,0.60,0.38");
checkRows = add_check(checkRows, "INT-09", "intensity", ...
    "two-level weakening is 0.02 for a=3:6 only", ...
    downTwoPass, "0.02 for a=3:6", "0.02 for a=3:6");
checkRows = add_check(checkRows, "INT-10", "intensity", ...
    "two-level enhancement is 0.005 for a=2:4 only", ...
    upTwoPass, "0.005 for a=2:4", "0.005 for a=2:4");
checkRows = add_check(checkRows, "INT-11", "intensity", ...
    "two-level enhancement is lower than two-level weakening", ...
    0.005 < 0.02, 0.005, "<0.02");
checkRows = add_check(checkRows, "INT-12", "intensity", ...
    "serialized intensity matrix exactly matches run-002 candidate", ...
    tables_numeric_equal(intensityRead,intensityCandidate,rowTolerance), ...
    "exact matrix", "exact matrix");
...
checkRows = add_check(checkRows, "LFW-02", "lfw", ...
    "lfw transitions use only -1, 0, +1, or +2", ...
    all(ismember(lfwRead.to_lfw-lfwRead.from_lfw,[-1;0;1;2])), ...
    strjoin(string(unique(lfwRead.to_lfw-lfwRead.from_lfw)).',','), ...
    "-1,0,1,2");
checkRows = add_check(checkRows, "LFW-03", "lfw", ...
    "lfw rows match the specified run-002 probabilities", ...
    abs(find_prob(lfwRead,'from_lfw',0,'to_lfw',0)-0.23)<=rowTolerance && ...
    abs(find_prob(lfwRead,'from_lfw',0,'to_lfw',1)-0.75)<=rowTolerance && ...
    abs(find_prob(lfwRead,'from_lfw',0,'to_lfw',2)-0.02)<=rowTolerance && ...
    abs(find_prob(lfwRead,'from_lfw',1,'to_lfw',0)-0.05)<=rowTolerance && ...
    abs(find_prob(lfwRead,'from_lfw',1,'to_lfw',1)-0.18)<=rowTolerance && ...
    abs(find_prob(lfwRead,'from_lfw',1,'to_lfw',2)-0.75)<=rowTolerance && ...
    abs(find_prob(lfwRead,'from_lfw',1,'to_lfw',3)-0.02)<=rowTolerance && ...
    abs(find_prob(lfwRead,'from_lfw',2,'to_lfw',1)-0.05)<=rowTolerance && ...
    abs(find_prob(lfwRead,'from_lfw',2,'to_lfw',2)-0.20)<=rowTolerance && ...
    abs(find_prob(lfwRead,'from_lfw',2,'to_lfw',3)-0.75)<=rowTolerance && ...
    abs(find_prob(lfwRead,'from_lfw',3,'to_lfw',2)-0.05)<=rowTolerance && ...
    abs(find_prob(lfwRead,'from_lfw',3,'to_lfw',3)-0.95)<=rowTolerance, ...
    "all four rows exact", "specified run-002 matrix");
checkRows = add_check(checkRows, "LFW-04", "lfw", ...
    "lfw invalid boundary mass is added to stay", lfwExactPass, ...
    "exact generated matrix", "invalid mass added to stay");
...
for a=2:5
    ...
    fprintf(fid,['- a=%d: enhancement %.12g -> %.12g; weakening ' ...
        '%.12g -> %.12g; stay %.12g -> %.12g; two-level weakening ' ...
        '%.12g; two-level enhancement %.12g.\n'],a,q.legacy_up, ...
        q.candidate_up,q.legacy_down,q.candidate_down,q.legacy_stay, ...
        q.candidate_stay,q.candidate_down_two,q.candidate_up_two);
end
fprintf(fid,'- a=6: 6->4=0.02, 6->5=0.60, and 6->6=0.38.\n\n');
...
fprintf(fid,['- base backward/stay/forward-one/forward-two probabilities: ' ...
    '%.2f/%.2f/%.2f/%.2f.\n'],lfwKernelProb(1),lfwKernelProb(2), ...
    lfwKernelProb(3),lfwKernelProb(4));
fprintf(fid,'- invalid boundary probability is added to the stay state.\n');
