...
config.outputDir = fullfile(moduleDir, 'output', ...
    'stage2_damage_persistence_smoke');
...
config.damageModes = ["independent_snapshot", "persistent_damage"];
...
config.executedSourceFiles = {
    fullfile(thisDir, 'run_stage2_damage_persistence_smoke_h2.m'), ...
    fullfile(thisDir, 'evaluate_damage_persistence_mode_h2.m'), ...
    fullfile(thisDir, 'compare_independent_persistent_damage_h2.m'), ...
    fullfile(thisDir, 'write_damage_persistence_diagnostics_h2.m')};

if isfolder(config.outputDir)
    existing = dir(config.outputDir);
    existing = existing(~ismember({existing.name}, {'.', '..'}));
    if ~isempty(existing)
        error('run_stage2_damage_persistence_smoke_h2:OutputExists', ...
...
result = evaluate_damage_persistence_mode_h2(config, foundation);
[summaryTbl, comparisonTbl, checksTbl, diagnostics] = ...
    compare_independent_persistent_damage_h2(result, config);
write_damage_persistence_diagnostics_h2(result.by_slice, summaryTbl, ...
    comparisonTbl, checksTbl, diagnostics, config);

fprintf('\nW1-W3 damage persistence smoke test finished.\n');
fprintf('Output directory:\n%s\n', config.outputDir);
fprintf('Common random numbers pass: %d\n', diagnostics.common_random_numbers_pass);
fprintf('Persistent recommendation: %s\n', diagnostics.B3_recommendation);
