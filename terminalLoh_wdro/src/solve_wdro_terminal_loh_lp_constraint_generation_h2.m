function sol = solve_wdro_terminal_loh_lp_constraint_generation_h2( ...
        D, A, C, Cap, M, rho, config)
%SOLVE_WDRO_TERMINAL_LOH_LP_CONSTRAINT_GENERATION_H2 Exact exchange experiment.
%
% This experimental solver keeps the existing TerminalLOH LP unchanged
% except that Wasserstein pair constraints are generated on demand. It does
% not build or retain an R-by-R distance matrix. Every separation pass scans
% all ordered scenario pairs in blocks.
...
baseTic = tic;
[baseModel, idx] = build_base_model(D, A, C, Cap, M, rho, config);
...
x = lastResult.x;
sol.exitflag = 1;
sol.objective_value = lastResult.objval;
sol.T = x(idx.T);
sol.y = x(idx.y);
sol.u = x(idx.u);
sol.L = x(idx.L);
...
function [base, idx] = build_base_model(D, A, C, Cap, M, rho, config)
[R, N] = size(D);
I = numel(Cap);
Ceff = C;
