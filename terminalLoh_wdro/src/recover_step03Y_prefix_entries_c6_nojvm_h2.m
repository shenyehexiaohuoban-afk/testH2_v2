function [entries, context] = recover_step03Y_prefix_entries_c6_nojvm_h2( ...
...
for kk = 1:numel(sampleSizes)
    ...
    audit = struct('all_final_DAC_exact', all(stateAudit.passed), ...
        ...
        'all_stream_hashes_match', false, ...
        'all_stream_identity_replay_match', all(stateAudit.passed), ...
        'stream_identity_replay_count', sum(stateAudit.passed), ...
...
audit = struct('all_final_DAC_exact', false, 'max_DAC_error', Inf, ...
    ...
    'all_stream_identity_replay_match', false, ...
    'stream_identity_replay_count', 0, ...
...
if isfield(audit, 'state_audit')
    ...
    auditRows(auditIndex, :) = {stateId, ...
        audit.all_final_DAC_exact && audit.all_stream_identity_replay_match, ...
        audit.max_DAC_error, ""};
end
end
...
for ss = 1:numel(states)
    ...
    if ~streamPass
        error('recover_step03Y_prefix_entries_h2:StreamMismatch', ...
            'Frozen random-stream seed-map row is missing.');
    end
...
audit = struct('all_final_DAC_exact', ...
    ...
    'all_stream_hashes_match', false, 'stream_match_count', 0, ...
    'all_stream_identity_replay_match', streamMatches == numel(states), ...
    'stream_identity_replay_count', streamMatches);
end
