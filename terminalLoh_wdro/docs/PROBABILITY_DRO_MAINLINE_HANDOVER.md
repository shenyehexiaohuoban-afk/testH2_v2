...
## 28. Step-04C-C5C accepted eta response and resilience saturation audit

Step-04C-C5C accepted `run-003` is at `results/task-002-stage2b-b3-smoke/52-eta-response-saturation-audit/run-003/`. It independently optimizes state19 at the frozen eta grid `[0,0.0003,0.001,0.003,0.01,0.03]` under the unchanged C5B yuan-consistent primary objective. It does not run the MSP or 35 states, change economic parameters or Pearson divergence, use validation data for optimization, introduce a CVaR objective, or modify the C5B/formal solvers. `C*y` remains a strict lexicographic secondary tie-break only.

The eta response is:

- eta `0`: `T=[222.840465864768,119.193737555574,36.003600360036,133.376974061042]`, total `511.414777841420 kg`, nominal EENS `113.420000000000 kWh`, nominal economic total `24571.3079460394 yuan`;
- eta `0.0003`: `T=[223.658693606249,121.102597522249,36.549314895204,135.832334729837]`, total `517.142940753539 kg`, nominal EENS `110.821526499780 kWh`, nominal economic total `24575.7024923112 yuan`;
- eta `0.001`: `T=[223.659164599841,121.377845308717,36.550298187626,137.467568714779]`, total `519.054876810963 kg`, nominal EENS `109.995740518659 kWh`, nominal economic total `24580.0762488336 yuan`;
- eta `0.003`: `T=[225.290752999455,121.925712142017,41.462552723036,137.464508019752]`, total `526.143525884259 kg`, nominal EENS `107.057013081324 kWh`, nominal economic total `24604.8978899629 yuan`;
- eta `0.01`: `T=[226.659147502450,121.966076719698,44.414041094008,139.650354720371]`, total `532.689620036528 kg`, nominal EENS `104.504173496184 kWh`, nominal economic total `24639.0870527584 yuan`;
- eta `0.03`: `T=[231.297621604390,121.918688071569,50.095285071282,139.650041539934]`, total `542.961636287175 kg`, nominal EENS `101.135543950904 kWh`, nominal economic total `24737.3430002616 yuan`.

No site binds capacity. C5B eta `0`, `0.003`, and `0.01` reproduce exactly: TerminalLOH, total inventory, nominal EENS, nominal economic total, LB, and UB have zero recorded difference. All six optimization cases are OPTIMAL. Positive-eta certified relative gaps are at most `9.6642e-09`; strong duality, probability conservation/nonnegativity, Pearson divergence, fixed-T reconstruction, lexicographic preservation, mechanical residuals, and `EENS=shortage*18.3315` all pass.

Validation evaluates all six decisions without reoptimization on the three separate C2 independent-path datasets, the three C3 combined-medium seeds, the three C3 combined-strong seeds, and the isolated state19 27-path/135-replica pressure set. The C2/C3 datasets retain separate probability laws. The pressure set has no empirical probability and remains outside the objective.

The adjacent eta `0.01 -> 0.03` segment adds `10.272016250634 kg` and `334.060016 yuan` of preparation cost. Mean EENS improves by `3.368629545279 kWh` nominally, `3.220110854631 kWh` across C2, `6.972565506106 kWh` under combined-medium, `10.897538112495 kWh` under combined-strong, and `27.558840730870 kWh` on the pressure set. Pressure q95 shortage improves by `3.045250289670 kg`; pressure q99.5 and maximum shortage do not change. The nominal EENS gain per added kg declines from `0.389979` for eta `0.003 -> 0.01` to `0.327942` for `0.01 -> 0.03`, so there is diminishing nominal marginal productivity even though strong-shift value remains.

The optimized adversarial distribution moves probability materially but not catastrophically. At eta `0.03`, divergence is `0.03`, total variation distance is `0.0385846`, ESS is `14563.1068`, and the maximum probability is `3.60231` times nominal. Total mass on the highest-loss `0.1%/0.5%/1%/5%` scenarios changes from `0.001/0.005/0.01/0.05` to `0.0031754/0.0129436/0.0227819/0.0811626`. The serviceable-shortage probability mass increases by `0.0383879`, showing that the adversary primarily emphasizes shortage scenarios where extra inventory can still affect service.

The SAA structural categories are A no shortage `12799/15000`, B reachable inventory-responsive shortage `214/15000`, C partial road-limited shortage `252/15000`, and D fully unreachable or inventory-invariant road floor `1735/15000`. Category D mean shortage stays exactly `31.341620 kg` at all six eta values. The global maximum-shortage scenario is category C. The probability diagnostic's stricter entirely-unreachable mass is zero; it must not be conflated with category D, which also contains inventory-invariant road-floor cases.

Development `run-001` failed at eta `0.0003` with MATLAB native heap corruption. `run-002` completed eta `0.001` and wrote passing outputs but returned the same nonzero exit during MATLAB teardown. Both are preserved locally and excluded from Git. The accepted C5C-only decomposition copy applies the frozen single-thread setting to master LPs as well as recourse LPs; C5B and formal solvers remain unchanged. `run-003` repeated the workflow from frozen inputs and all 19 isolated processes exited zero.

## 29. Current decision boundary after Step-04C-C5C
